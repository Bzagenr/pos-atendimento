...
# Estados da conversa
LGPD, CPF, DATA_NASCIMENTO, ESCOLHA_CONSULTA = range(4)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Verifica se já consentiu antes
    try:
        resp = requests.get(
    ...
    # Primeiro acesso — exibe termo LGPD
...
async def receber_data_nascimento(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if response.status_code == 404:
            ...
            elif "nenhuma consulta" in detail.lower():
                await update.message.reply_text(
                    "ℹ️ Nenhuma consulta finalizada encontrada para este CPF.\n"
                    "Em caso de dúvidas, fale com a recepção."
                )
            else:
                await update.message.reply_text(
                    "ℹ️ Nenhuma informação encontrada.\n"
                    "Em caso de dúvidas, fale com a recepção."
                )
            return ConversationHandler.END

        # Registra consentimento LGPD e chat_id na API
        requests.patch(
        ...
        await update.message.reply_text(
            "⏳ O servidor demorou para responder. Tente novamente com /start"
        )
        return ConversationHandler.END
    except Exception:
        await update.message.reply_text(
            "⚠️ Ocorreu um erro. Tente novamente com /start"
        )
        return ConversationHandler.END
...
async def escolher_consulta(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    consulta_escolhida = next(
        (c for c in consultas if escolha.startswith(c["data"])),
        None
    )
    ...
    try:
        ...
        await exibir_consulta(update, response.json())
    except Exception:
...
async def exibir_consulta(update: Update, data: dict):
    ...
    if receita["possui_receita"] and receita["medicamentos"]:
        texto = "💊 *Receita médica:*\n"
        for med in receita["medicamentos"]:
            ...
            if med["duracaoDias"]:
                texto += f"\n   Duração: {med['duracaoDias']} dias"
            if med["observacao"]:
                texto += f"\n   Obs: {med['observacao']}"
        await update.message.reply_text(texto, parse_mode="Markdown")
    else:
        await update.message.reply_text("💊 Nenhuma receita registrada para esta consulta.")
